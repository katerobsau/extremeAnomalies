...
- `r percent(sum(l>270)/length(l))` of stations have more than $270$ years of data
:::
...
tar_qplot(id, period = period, data = df_prcp_zero, n = 20)
```

## Extreme missings
```{r}

df_extreme_missing <- filter(df_prcp_zero, flag_missing_extreme)
...
### Southeast Queensland
- `r nrow(df_extreme_zero)` missing observations are flagged as extreme values
